...
- **batch_enthalpy.py**：批量绘图脚本，利用 Plotly + Kaleido 生成二元曲线、三元等值线以及四元预览（含固定元素的三元投影）。
...
3. 四元 ΔH_mix 预览：输入四种元素后，以可旋转的等边四面体展示组成空间与混合焓分布。可在预览后固定其中一个元素的浓度（默认步长 1%，固定元素容差 ±0.5%），其余三元素以 0.1% 步长重新采样并计算 ΔH 生成三元投影 PNG/HTML。
